...
-- | Postgres health SQL: reads over the in-memory @pg_stat_*@ views.
...
-- | One row of database-wide health. All counters are cumulative, so rate() them.
pgDbHealthSQL :: Text
pgDbHealthSQL =
  [text|
    SELECT
      ...
      d.numbackends::int8,
      a.active::int8, a.idle::int8, a.idle_in_txn::int8, a.blocked::int8,
...
-- | Per-table churn for the arbiter schema, one row per table.
pgTableHealthSQL :: Text
pgTableHealthSQL =
  [text|
    SELECT
