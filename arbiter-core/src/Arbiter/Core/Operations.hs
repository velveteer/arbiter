...
likeEscape = T.concatMap escape
  where
    escape ch = if ch `elem` likeMeta then T.pack ['\\', ch] else T.singleton ch
...
payloadColumns payloadValue =
  let rlKey = runRateLimitFor payloadValue (rateLimitFor @payload)
   ...
   in PayloadColumns
        ...
        , pcRateLimitCost = rateLimitCost payloadValue
...
dedupBatch = toList . snd . foldl' step (Map.empty, Seq.empty)
  where
    step (!seen, !rows) job = case dedupKeyText (JT.dedupKey job) of
      ...
      Just key -> case Map.lookup key seen of
        Nothing -> (Map.insert key (Seq.length rows) seen, rows |> job)
        Just idx
...
chunksOfNE size (leader :| others) = go (leader : others)
  where
    go [] = []
    go (next : more) =
      let (chunk, rest) = splitAt (size - 1) more
       in (next :| chunk) : go rest
...
ackJobInner schemaName tableName job =
  ackJobUsing schemaName tableName (Tmpl.smartAckJobSQL (archivesOnAck job) schemaName tableName) job

-- | A pool's statements, rendered once. A call binds only its parameters.
data JobStatements = JobStatements
  { statementsSchema :: SchemaName
  , statementsTable :: TableName
  , claimBatchSize :: Int
  , claimFor :: Int -> Q.Query (JobRead Value)
  -- ^ The claim at a capacity, with this pool's claimant bound.
  , ackPlain :: Int64 -> Int64 -> Q.Query Int64
  , ackArchiving :: Int64 -> Int64 -> Q.Query Int64
  , ackBatchPlain :: [Int64] -> [Int64] -> Q.Query Int64
  , ackBatchArchiving :: [Int64] -> [Int64] -> Q.Query Int64
  }
...
-- | 'ackJobInner' over a pool's staged statements.
ackJobWith :: (MonadArbiter m) => JobStatements -> JobRead payload -> m Int64
ackJobWith statements job =
  ackJobUsing (statementsSchema statements) (statementsTable statements) ack job
  where
    ack = if archivesOnAck job then ackArchiving statements else ackPlain statements

ackJobUsing
  :: (MonadArbiter m) => SchemaName -> TableName -> (Int64 -> Int64 -> Q.Query Int64) -> JobRead payload -> m Int64
ackJobUsing schemaName tableName ack job = do
  lockJobParents schemaName tableName [parentId job]
  countOr0Prepared (ack (primaryKey job) (claimSeq job))
...
ackJobsBatchInner
  :: forall m payload
   . (MonadArbiter m)
  => SchemaName -> TableName -> [JobRead payload] -> m [Int64]
ackJobsBatchInner schemaName tableName jobs =
  ackJobsBatchUsing schemaName tableName (Tmpl.smartAckJobsBatchSQL (any archivesOnAck jobs) schemaName tableName) jobs

-- | 'ackJobsBatchInner' over a pool's staged statements.
ackJobsBatchWith :: (MonadArbiter m) => JobStatements -> [JobRead payload] -> m [Int64]
ackJobsBatchWith statements jobs =
  ackJobsBatchUsing (statementsSchema statements) (statementsTable statements) ack jobs
  where
    ack = if any archivesOnAck jobs then ackBatchArchiving statements else ackBatchPlain statements

ackJobsBatchUsing
  :: (MonadArbiter m) => SchemaName -> TableName -> ([Int64] -> [Int64] -> Q.Query Int64) -> [JobRead payload] -> m [Int64]
ackJobsBatchUsing _ _ _ [] = pure []
ackJobsBatchUsing schemaName tableName ack jobs = do
  lockJobParents schemaName tableName (map parentId jobs)
  MA.executeQueryPrepared (ack (map primaryKey jobs) (map claimSeq jobs))
...
instance ToJSON QueueStats where
  toJSON stats =
    object
      [ "totalJobs" .= totalJobs stats
      , "readyJobs" .= readyJobs stats
      , "inFlightJobs" .= inFlightJobs stats
      , "scheduledJobs" .= scheduledJobs stats
      , "backoffJobs" .= backoffJobs stats
...
instance FromJSON QueueStats where
  parseJSON = withObject "QueueStats" $ \obj ->
    QueueStats
      <$> obj .: "totalJobs"
      <*> obj .: "readyJobs"
      <*> obj .: "inFlightJobs"
...
instance ToJSON QueueOverview where
  toJSON overview =
    object
      [ "queue" .= overviewQueue overview
      , "stats" .= overviewStats overview
      , "paused" .= overviewQueuePaused overview
      , "workersLive" .= overviewWorkersLive overview
      , "workersPaused" .= overviewWorkersPaused overview
      ]

instance FromJSON QueueOverview where
  parseJSON = withObject "QueueOverview" $ \obj ->
    QueueOverview
      <$> obj .: "queue"
      <*> obj .: "stats"
      <*> obj .: "paused"
...
countJobsByGroup schemaName tableName key =
  countJobsFiltered schemaName tableName [Tmpl.FilterGroupKey key]
...
instance ToJSON GroupsCursor where
  toJSON (GroupsCursor window emptied) =
    object (catMaybes [("window" .=) <$> window, ("emptied" .=) <$> emptied])

instance FromJSON GroupsCursor where
  parseJSON = withObject "GroupsCursor" $ \obj ->
    GroupsCursor <$> obj .:? "window" <*> obj .:? "emptied"
...
resumeCursor Nothing Nothing = Nothing
...
refreshGroupsForQueue
  ...
  -> m GroupsPass
refreshGroupsForQueue schemaName tableName batch resume = do
  (rewritten, windowEnd, nextEmptied) <- withDbTransaction $ do
  ...
  let repaired = fromRight [] repair
      -- A filled repair batch left keys behind.
      nextWindow = if length repaired >= batch then lastRow (map fst repaired) else windowEnd
  pure
    GroupsPass
  ...
  where
    cursor = resume >>= groupsWindowFrom
    emptiedCursor = resume >>= groupsEmptiedFrom

-- | The last row, in the order the database returned it.
lastRow :: [a] -> Maybe a
lastRow = foldl' (\_ row -> Just row) Nothing
...
refreshAllGroups schemaName queues cursors = do
  ...
  let rewritten = sum [passRewritten pass | (_, pass) <- refreshed]
      unrepaired = [tbl | (tbl, pass) <- refreshed, passRepairFailed pass]
      resumed = foldl' (\acc (tbl, pass) -> Map.alter (const (passResume pass)) tbl acc) cursors refreshed
...
refreshAllGroupsFully
  ...
  -> [TableName]
  -> m (Int64, [Text])
refreshAllGroupsFully schemaName queues = do
  (walked, failed) <- sweepEachQueue walk schemaName queues
  let unrepaired = [tbl | (tbl, (_, True)) <- walked]
  pure (sum [count | (_, (count, _)) <- walked], failed <> unrepaired)
  where
    walk schema tbl = go 0 False Nothing
      where
        go acc failedRepair cursor = do
          pass <- refreshGroupsForQueue schema tbl groupsRefreshBatch cursor
          let total = acc + passRewritten pass
              failedYet = failedRepair || passRepairFailed pass
...
mergeRawChildResults results failures mSnapshot =
  ...
  where
    base = case mSnapshot of
      Just val | Success snapshotMap <- fromJSON val -> snapshotMap
      _ -> Map.empty
