...
| `withDbTransaction` | transaction, or a savepoint when nested; [worker configuration](../worker/configuration.md) |
| `runHandlerWithConnection` | check out a connection and run a handler |
| `getListener` | shared `LISTEN/NOTIFY` listener, or `Nothing` for polling; [wakeups](../worker/wakeups.md) |
| `executeQueryPrepared` | optional; defaults to `executeQuery`. Claims use it. Prepared once per connection when overridden; [benchmarks](index.md). |
